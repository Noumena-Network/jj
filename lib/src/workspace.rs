...
use crate::eden_fs_working_copy::EdenFsWorkingCopy;
use crate::eden_fs_working_copy::EdenFsWorkingCopyFactory;
...
impl Workspace {
    ...
    pub fn working_copy_mut(&mut self) -> &mut dyn WorkingCopy {
...
/// Register the EdenFS working-copy factory in an existing map.
///
/// This is intentionally opt-in while `EdenFsWorkingCopy` is a state-tracking
/// scaffold. Production Mononoke-mode initializers must only call this once
/// checkout, snapshot, and sparse updates are backed by real EdenFS APIs.
pub fn register_edenfs_working_copy_factory(factories: &mut WorkingCopyFactories) {
    factories.insert(
        EdenFsWorkingCopy::name().to_owned(),
        Box::new(EdenFsWorkingCopyFactory {}),
    );
}
