...
/// Encode a JJ view using the canonical simple-op-store protobuf payload.
pub fn encode_view_to_proto_bytes(view: &View) -> (ViewId, Vec<u8>) {
    let id = ViewId::new(blake2b_hash(view).to_vec());
    (id, view_to_proto(view).encode_to_vec())
}

/// Decode a JJ view from a canonical simple-op-store protobuf payload.
pub fn decode_view_from_proto_bytes(id: &ViewId, bytes: &[u8]) -> OpStoreResult<View> {
    let proto = crate::protos::simple_op_store::View::decode(bytes)
        .map_err(|err| to_read_error(err.into(), id))?;
    let view = view_from_proto(proto).map_err(|err| to_read_error(err.into(), id))?;
    let actual_id = ViewId::new(blake2b_hash(&view).to_vec());
    if &actual_id != id {
        return Err(to_read_error(
            io::Error::new(
                ErrorKind::InvalidData,
                format!(
                    "decoded view hash {} did not match requested id {}",
                    actual_id.hex(),
                    id.hex()
                ),
            )
...
/// Encode a JJ operation using the canonical simple-op-store protobuf payload.
pub fn encode_operation_to_proto_bytes(
    operation: &Operation,
) -> OpStoreResult<(OperationId, Vec<u8>)> {
    if operation.parents.is_empty() {
        return Err(OpStoreError::WriteObject {
            object_type: "operation",
            source: io::Error::new(
                ErrorKind::InvalidInput,
                "non-root operation must have at least one parent",
            )
            .into(),
        });
    }
    let id = OperationId::new(blake2b_hash(operation).to_vec());
    Ok((id, operation_to_proto(operation).encode_to_vec()))
}

/// Decode a JJ operation from a canonical simple-op-store protobuf payload.
pub fn decode_operation_from_proto_bytes(
    id: &OperationId,
    bytes: &[u8],
) -> OpStoreResult<Operation> {
    let proto = crate::protos::simple_op_store::Operation::decode(bytes)
        .map_err(|err| to_read_error(err.into(), id))?;
    let mut operation = operation_from_proto(proto).map_err(|err| to_read_error(err.into(), id))?;
    let actual_id = OperationId::new(blake2b_hash(&operation).to_vec());
    if &actual_id != id {
        return Err(to_read_error(
            io::Error::new(
                ErrorKind::InvalidData,
                format!(
                    "decoded operation hash {} did not match requested id {}",
                    actual_id.hex(),
                    id.hex()
                ),
            )
    ...
    if operation.parents.is_empty() {
        operation.parents.push(OperationId::from_bytes(&[0; OPERATION_ID_LENGTH]));
    }
    Ok(operation)
}
...
impl OpStore for SimpleOpStore {
    ...
    async fn read_view(&self, id: &ViewId) -> OpStoreResult<View> {
        ...
        decode_view_from_proto_bytes(id, &buf)
    }
    ...
    async fn write_view(&self, view: &View) -> OpStoreResult<ViewId> {
        ...
        let (id, data) = encode_view_to_proto_bytes(view);
        temp_file
            .as_file()
            .write_all(&data)
    ...
    async fn read_operation(&self, id: &OperationId) -> OpStoreResult<Operation> {
        ...
        decode_operation_from_proto_bytes(id, &buf)
    }
    ...
    async fn write_operation(&self, operation: &Operation) -> OpStoreResult<OperationId> {
        ...
        let (id, data) = encode_operation_to_proto_bytes(operation)?;
        temp_file
            .as_file()
            .write_all(&data)
            .context(temp_file.path())
